...
function getTileWidth(viewportWidth: number) {
	const widthRatio = viewportWidth <= COMPACT_LAYOUT_MAX_WIDTH
		? 0.42
		: viewportWidth < MEDIUM_LAYOUT_MIN_WIDTH
			? 0.32
			: 0.21;

	return Math.min(TILE_MAX_WIDTH, Math.max(TILE_MIN_WIDTH, viewportWidth * widthRatio));
}
...
function getMinimumTilesPerLine(viewportWidth: number) {
	...
	return Math.ceil((viewportWidth + tileStep - lineStart - tileWidth) / tileStep) + 1;
}
...
export function ProjectList() {
	...
	const linePositions = isCompactLayout ? LINE_POSITIONS.compact : LINE_POSITIONS.wide;
	const minimumTilesPerLine = getMinimumTilesPerLine(viewportWidth);
	const projectLines = linePositions.map((_, lineIndex) =>
		createLoopItems(
			projectsData.filter((_, projectIndex) => projectIndex % linePositions.length === lineIndex),
			minimumTilesPerLine
		)
	);
	...
	useGSAP(
		() => {
			const dragTargets = gsap.utils.toArray<HTMLElement>("[data-project-drag]");
			let activeZIndex = 40;
			...
			const lines = gsap.utils.toArray<HTMLElement>("[data-project-line]").map((line, lineIndex) => {
				const tiles = gsap.utils.toArray<HTMLElement>("[data-project-tile]", line);
			...
			motion.add("(prefers-reduced-motion: no-preference)", () => {
				lines.forEach(({ initialPhase, line, loopState, positionTiles, tileCount }) => {
		...
		{ scope: sectionRef, dependencies: [isCompactLayout, minimumTilesPerLine], revertOnUpdate: true }
	);
	...
	return (
		...
		>
			...
			{projectLines.map((lineProjects, lineIndex) => (
				...
				>
					{lineProjects.map(({ item: project, isDuplicate, loopIndex }, itemIndex) => {
						const shape = TILE_SHAPE_VARIANTS[(itemIndex + lineIndex * 2) % TILE_SHAPE_VARIANTS.length];
						const draggableClassName = "group block h-full w-full touch-none cursor-grab select-none active:cursor-grabbing";
						...
						return (
							...
							>
								...
								>
									<figure className={`relative h-full w-full overflow-hidden bg-neutral-900 ${shape}`}>
										<img
			...
			<div className="absolute inset-x-0 bottom-0 z-30 grid grid-cols-2 gap-8 p-6 md:grid-cols-3">
				...
				<p className="max-w-72 text-2xs leading-3 font-medium uppercase md:col-start-3">
					Responsive websites, web applications, and client work across different industries.
				</p>
			</div>
		</section>
	);
